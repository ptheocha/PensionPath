...
// ═══════════════════════════════════════════════
// PLAN DATA (Ontario Only — verified March 2026)
// ═══════════════════════════════════════════════
const PLANS = {
  hoopp: {
    name: "HOOPP", full: "Healthcare of Ontario Pension Plan", who: "Healthcare sector employees",
    normalRetAge: 60, earliestRetAge: 55, colaType: "partial", colaHistorical: 2.0,
    hasBridge: true, bridgeEndAge: 65, survivorPct: 66.67,
    survivorOptions: [66.67, 80, 100],
    cvMaxAge: 55, // Must be under 55 to take CV
  },
  omers: {
    name: "OMERS", full: "Ontario Municipal Employees Retirement System", who: "Municipal & local government",
    normalRetAge: 65, earliestRetAge: 55, colaType: "partial", colaHistorical: 2.0,
    hasBridge: true, bridgeEndAge: 65, survivorPct: 66.67,
    survivorOptions: [66.67], // OMERS does not offer increased survivor options
    cvMaxAge: 55, // Must be >10 years from NRA 65
  },
  otpp: {
    name: "OTPP", full: "Ontario Teachers' Pension Plan", who: "Teachers & education workers",
    normalRetAge: 65, earliestRetAge: 50, colaType: "partial", colaHistorical: 2.0,
    hasBridge: true, bridgeEndAge: 65, survivorPct: 60,
    survivorOptions: [50, 55, 60, 65, 70, 75],
    cvMaxAge: 50, // Must be under 50 to take CV
  },
  optrust: {
    name: "OPTrust", full: "OPSEU Pension Trust", who: "Ontario public service (OPSEU)",
    normalRetAge: 65, earliestRetAge: 55, colaType: "guaranteed", colaHistorical: 2.0,
    hasBridge: true, bridgeEndAge: 65, survivorPct: 60,
    survivorOptions: [50, 55, 60, 65, 70, 75],
    cvMaxAge: 55, hasInsuredBenefits: true,
  },
  pspp: {
    name: "PSPP", full: "Public Service Pension Plan", who: "Provincial government employees",
    normalRetAge: 65, earliestRetAge: 55, colaType: "guaranteed", colaHistorical: 2.0,
    hasBridge: true, bridgeEndAge: 65, survivorPct: 60,
    survivorOptions: [60, 65, 70, 75],
    cvMaxAge: 55, hasInsuredBenefits: true,
  },
  caat: {
    name: "CAAT", full: "Colleges of Applied Arts & Technology", who: "College & polytechnic employees",
    normalRetAge: 65, earliestRetAge: 50, colaType: "conditional", colaHistorical: 2.0,
    hasBridge: true, bridgeEndAge: 65, survivorPct: 60,
    survivorOptions: [60, 75, 100],
    cvMaxAge: 48, // Under 50 after 24-month extension
    hasMembershipExtension: true,
  },
};
...
// ═══════════════════════════════════════════════
const PV_FACTORS = {50:9.4,51:9.6,52:9.8,53:10,54:10.2,55:10.4,56:10.6,57:10.8,58:11,59:11.3,60:11.5,61:11.7,62:12,63:12.2,64:12.3,65:12.4};
...
// Total tax through all brackets up to a given income
function totalTaxFed(inc){let tax=0,prev=0;for(const b of FED){const t=Math.min(inc,b.l)-prev;if(t<=0)break;tax+=t*b.r;prev=b.l}return tax}
function totalTaxON(inc){let tax=0,prev=0;for(const b of ON){const t=Math.min(inc,b.l)-prev;if(t<=0)break;tax+=t*b.r;prev=b.l}return tax}
function totalTax(inc){return totalTaxFed(Math.max(0,inc))+totalTaxON(Math.max(0,inc))}
// Incremental tax: exact tax on 'amount' stacked on top of 'baseIncome'
function incrementalTax(baseIncome,amount){if(amount<=0)return 0;return totalTax(baseIncome+amount)-totalTax(baseIncome)}
// Effective rate on the incremental amount (for display)
function effectiveRate(baseIncome,amount){if(amount<=0)return 0;return incrementalTax(baseIncome,amount)/amount}
// Marginal rate at a given income level (for reference)
...
function Tooltip({text}){
  ...
  return(
    <span style={{position:"relative",display:"inline-flex",alignItems:"center",marginLeft:6}}
      ...
      <span onClick={handleClick} onTouchEnd={handleTouch} role="button" tabIndex={0}
        style={{width:22,height:22,borderRadius:"50%",background:"#403e3a",border:"1px solid #524f44",
...
function Input({label,value,onChange,prefix,tooltip,hint}){
  const formatDisplay=(v)=>{
    if(v===""||v==="-")return v;
    if(v.endsWith("."))return Number(v.slice(0,-1)).toLocaleString("en-CA")+".";
    const dotIdx=v.indexOf(".");
    if(dotIdx>=0){const ip=v.slice(0,dotIdx),dp=v.slice(dotIdx);return(ip===""||ip==="-"?ip:Number(ip).toLocaleString("en-CA"))+dp}
    return Number(v).toLocaleString("en-CA");
  };
  const displayVal=formatDisplay(value);
  const handleChange=(e)=>{const raw=e.target.value.replace(/,/g,"");if(raw===""||raw==="-"){onChange(raw);return}if(/^-?\d*\.?\d*$/.test(raw))onChange(raw)};
  return(
    <div style={{flex:"1 1 200px",minWidth:0,marginBottom:4}}>
      ...
      {hint&&<div style={{fontSize:10,color:"#636054",marginTop:4}}>{hint}</div>}
    </div>
  );
}
...
export default function PensionPath(){
  ...
  const [survivorElect,setSurvivorElect]=useState("");
  const [investFees,setInvestFees]=useState("0.3");
  const [taxDrag,setTaxDrag]=useState("0.7");
  ...
  const rrspN=isDetailed?(parseFloat(rrspRoom)||0):0;
  const spouseA=isDetailed?(parseFloat(spouseAge)||0):0;
  const feesN=isDetailed?(parseFloat(investFees)||0)/100:0;
  const taxDragN=isDetailed?(parseFloat(taxDrag)||0)/100:0;
  const survPct=isDetailed&&survivorElect?(parseFloat(survivorElect)):plan?.survivorPct||60;
  const totalMonthly=lifeN+(plan?.hasBridge?bN:0);
  const cvEligible=plan?a<(plan.cvMaxAge||55):true;
  const ready=planKey&&a>=18&&a<70&&cvN>0&&lifeN>0;

  // Set default survivor election when plan changes
  const prevPlanRef=useRef("");
  if(planKey!==prevPlanRef.current){prevPlanRef.current=planKey;if(plan&&!survivorElect)setSurvivorElect(String(plan.survivorPct))}
  ...
  const R=useMemo(()=>{
    ...
    const fullMonthly=lifeN+(plan.hasBridge?bN:0);
    const netReturn=investReturn/100-feesN;
    const invR=netReturn;

    // DEFER — pension lifetime value
    let dR=0,dM=0;
    for(let y=0;y<yInR;y++){
      const c=Math.pow(1+cola,yToR+y),inB=plan.hasBridge&&(retAge+y)<plan.bridgeEndAge;
      const m=inB?(fullMonthly*c):(lifeN*c);if(!y)dM=m;dR+=(m*12)/Math.pow(1+inf,yToR+y);
    }

    // DEFER — survivor value (detailed mode with spouse)
    let survivorValue=0;
    if(isDetailed&&spouseA>0){
      const memberDeathAge=lifeExp;
      const spouseLifeExp=Math.max(memberDeathAge,spouseA+(82-((spouseA>a)?0:2)));// estimate spouse lives to ~82 avg
      const spouseSurvivorYears=Math.max(0,Math.min(spouseLifeExp,100)-memberDeathAge);
      const survivorMonthly=lifeN*(survPct/100);// survivor gets % of lifetime pension (no bridge)
      for(let y=0;y<spouseSurvivorYears;y++){
        const c=Math.pow(1+cola,yToR+yInR+y);
        survivorValue+=(survivorMonthly*c*12)/Math.pow(1+inf,yToR+yInR+y);
      }
    }

    // INVEST (LIRA + RRSP + cash)
    const pvF=getPV(a),mtv=(lifeN*12)*pvF,lira=Math.min(total,mtv),exOver=Math.max(total-mtv,0);
    const rrspSheltered=Math.min(exOver,rrspN);// shelter excess in RRSP if room
    const taxableExcess=Math.max(exOver-rrspSheltered,0);
    const taxEx=incrementalTax(incN,taxableExcess),eRate=effectiveRate(incN,taxableExcess),net=total-taxEx;
    const atRet=net*Math.pow(1+invR,yToR),mr=invR/12,nM=yInR*12;
    const draw=mr>0?atRet*mr/(1-Math.pow(1+mr,-nM)):atRet/nM;
    const lockedPct=(lira+rrspSheltered)/total;// both LIRA and RRSP are registered
    const afterTax=draw*(1-0.25*lockedPct);
    let iR=0;for(let y=0;y<yInR;y++)iR+=(afterTax*12)/Math.pow(1+inf,yToR+y);

    // CASH — with tax drag on non-registered growth
    const cTax=incrementalTax(incN,total),cERate=effectiveRate(incN,total),cNet=total-cTax;
    const cashGrowthRate=invR-taxDragN;// reduce growth by tax drag
    const cAtRet=cNet*Math.pow(1+cashGrowthRate,yToR);
    const cMr=cashGrowthRate/12,cNM=yInR*12;
    const cDraw=cMr>0?cAtRet*cMr/(1-Math.pow(1+cMr,-cNM)):cAtRet/cNM;
    ...
    return{total,dR,dM,iR,iM:afterTax,iAtRet:atRet,cR,cM:cDraw,cNet,cAtRet,cTax,mtv,lira,exOver,taxEx,eRate,cERate,pvF,bk,yToR,fullMonthly,
      rrspSheltered,taxableExcess,survivorValue,survPct,netReturn:netReturn*100};
  },[ready,plan,a,cvN,exN,lifeN,bN,incN,investReturn,colaRate,lifeExp,isDetailed,rrspN,spouseA,survPct,feesN,taxDragN]);

  const best=R?(cvEligible?[{k:"defer",v:R.dR+(R.survivorValue||0)},{k:"invest",v:R.iR},{k:"cash",v:R.cR}]:[{k:"defer",v:R.dR+(R.survivorValue||0)}]).sort((x,y)=>y.v-x.v)[0].k:null;
  const maxV=R?(cvEligible?Math.max(R.dR+(R.survivorValue||0),R.iR,R.cR):(R.dR+(R.survivorValue||0))):0;

  // Compute closeness of top two results for confidence warning
  const resultsClose=R?(() => {
    const vals=[R.dR+(R.survivorValue||0),R.iR,R.cR].sort((a,b)=>b-a);
    return vals[0]>0 && (vals[0]-vals[1])/vals[0]<0.15;
  })():false;

  const scenarios=R?[
    {key:"defer",label:"Keep Your Pension",sub:`Collect at ${plan.normalRetAge} for life`,icon:"🛡️",color:"#4ecdc4",
      total:R.dR+(R.survivorValue||0),monthly:R.dM,
      details:[
        ...
        {l:"Survivor benefit",v:`${R.survPct}% to spouse`},
        ...(isDetailed&&R.survivorValue>0?[{l:"Survivor value (PV)",v:fmtK(R.survivorValue)}]:[]),
        {l:"Investment risk",v:"None"},
        {l:"Flexibility",v:"Low"},
      ]},
    ...(cvEligible?[{key:"invest",label:"Transfer & Invest",sub:"Tax-sheltered transfer to LIRA/RRSP",icon:"📈",color:"#d4a843",
      total:R.iR,monthly:R.iM,
      details:[
        {l:"Tax-free to LIRA",v:fmt(R.lira)},
        ...(isDetailed&&R.rrspSheltered>0?[{l:"Sheltered in RRSP",v:fmt(R.rrspSheltered)}]:[]),
        ...(R.taxableExcess>0?[{l:"Taxable cash portion",v:fmt(R.taxableExcess),warn:true},{l:`Tax on cash (${fmtPct(R.eRate)} eff.)`,v:`-${fmt(R.taxEx)}`,warn:true}]:[]),
        ...(R.exOver>0&&!isDetailed?[{l:"Taxable excess",v:fmt(R.exOver),warn:true},{l:`Tax on excess (${fmtPct(R.eRate)} eff.)`,v:`-${fmt(R.taxEx)}`,warn:true}]:[]),
        {l:`Grows to (at ${R.netReturn.toFixed(1)}%)`,v:fmtK(R.iAtRet)},
        {l:"Monthly at retirement",v:fmtFull(R.iM)},
        {l:"Investment risk",v:"Medium"},
        {l:"Flexibility",v:"Medium"},
      ]}]:[]),
    ...(cvEligible?[{key:"cash",label:"Cash Out & Invest",sub:"Withdraw as cash, pay tax, invest the rest",icon:"💵",color:"#e07a5f",
      total:R.cR,monthly:R.cM,
      details:[
        {l:"Gross withdrawal",v:fmt(R.total)},
        {l:`Tax on withdrawal (${fmtPct(R.cERate)} eff.)`,v:`-${fmt(R.cTax)}`,warn:true},
        {l:"Amount you invest",v:fmt(R.cNet)},
        {l:`Grows to (at ${(investReturn-feesN*100-(isDetailed?taxDragN*100:0)).toFixed(1)}%)`,v:fmtK(R.cAtRet)},
        {l:"Monthly at retirement",v:fmtFull(R.cM)},
        ...(isDetailed?[{l:"Tax drag applied",v:`${taxDrag}%/yr on non-registered growth`}]:[]),
    ...
    <div style={{minHeight:"100vh",background:"#24231e",color:"#f0e8d4",fontFamily:"'DM Sans',sans-serif",overflowX:"hidden"}}>
      ...
      <div style={{maxWidth:720,margin:"0 auto",padding:"36px 16px"}}>
        ...
        <div style={{display:"flex",justifyContent:"center",marginBottom:24}}>
          <div style={{display:"flex",background:"#302f29",borderRadius:10,border:"1px solid #403e3a",overflow:"hidden"}}>
            {[["quick","Quick Analysis"],["detailed","Detailed Analysis"]].map(([k,label])=>(
              <button key={k} onClick={()=>setMode(k)} style={{
                padding:"10px 22px",fontSize:13,fontWeight:600,border:"none",cursor:"pointer",
                fontFamily:"'DM Sans',sans-serif",transition:"all 0.2s",
                background:mode===k?"#d4a843":"transparent",color:mode===k?"#24231e":"#858070"
              }}>{label}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{background:"#2b2a25",border:"1px solid #403e3a",borderRadius:14,padding:"28px 24px",marginBottom:28}}>
          ...
          <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:10,marginBottom:20}}>
            {PLAN_ORDER.map(k=>{const p=PLANS[k];return(
              <button key={k} onClick={()=>{setPlanKey(k);setColaRate(PLANS[k].colaHistorical);setSurvivorElect(String(PLANS[k].survivorPct))}} style={{
          ...
          {plan&&(
            <div style={{marginBottom:28,background:"#38362e",borderRadius:10,overflow:"hidden",border:"1px solid #403e3a"}}>
              ...
              {[
                ...
                ["Survivor benefit",`${plan.survivorPct}% to spouse${plan.survivorOptions.length>1?` (up to ${Math.max(...plan.survivorOptions)}%)`:""}`],
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:"20px 24px",marginBottom:24}}>
            <Input label="Commuted value" value={cv} onChange={setCv} prefix="$" tooltip={plan?`The lump sum ${plan.name} is offering instead of your future pension. Found on your termination options package.`:"The lump sum your plan is offering. Select your plan above for details."}/>
            <Input label="Excess contributions" value={excess} onChange={setExcess} prefix="$" tooltip={plan?`A separate refund amount some ${plan.name} members receive. If listed on your termination package, enter it here. Otherwise leave blank.`:"A separate refund listed on some termination packages. Leave blank if not applicable."}/>
          </div>
          <div style={{display:"flex",flexWrap:"wrap",gap:"20px 24px",marginBottom:24}}>
            <Input label="Lifetime pension (monthly)" value={lifetime} onChange={setLifetime} prefix="$" tooltip={plan?`Your permanent monthly pension for life${plan.hasBridge?`, starting at age ${plan.normalRetAge}. This continues after the bridge ends at ${plan.bridgeEndAge}`:` starting at age ${plan.normalRetAge}`}. Found on your ${plan.name} termination statement.`:"The permanent monthly amount you receive for life. Select your plan for details."}/>
            {plan?.hasBridge&&(
          ...
          {/* ═══ DETAILED INPUTS ═══ */}
          {isDetailed&&(
            <div style={{marginTop:8,padding:24,background:"#302f29",borderRadius:12,border:"1px solid #403e3a",marginBottom:24}}>
              <div style={{fontSize:11,fontWeight:600,color:"#d4a843",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:16}}>Additional details</div>

              <div style={{display:"flex",flexWrap:"wrap",gap:"20px 24px",marginBottom:20}}>
                <Input label="Available RRSP room" value={rrspRoom} onChange={setRrspRoom} prefix="$" tooltip="Check your CRA Notice of Assessment. RRSP room lets you shelter more of the taxable excess from tax." hint="From your CRA Notice of Assessment"/>
                <Input label="Spouse's age" value={spouseAge} onChange={setSpouseAge} tooltip="Used to estimate the value of survivor pension benefits. Leave blank if no spouse." hint="Leave blank if no spouse"/>
              </div>

              {/* Survivor election */}
              {plan&&plan.survivorOptions.length>1&&(
                <div style={{marginBottom:20}}>
                  <div style={{display:"flex",alignItems:"center",fontSize:12,fontWeight:600,color:"#c4b99a",marginBottom:10}}>
                    Survivor benefit election
                    <Tooltip text={`${plan.name} lets you choose how much pension continues to your spouse. Higher % = lower monthly pension for you. Election is made at retirement.`}/>
                  </div>
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    {plan.survivorOptions.map(pct=>(
                      <button key={pct} onClick={()=>setSurvivorElect(String(pct))} style={{
                        padding:"8px 14px",borderRadius:8,fontSize:13,fontWeight:600,cursor:"pointer",
                        border:`1.5px solid ${parseFloat(survivorElect)===pct?"#d4a843":"#403e3a"}`,
                        background:parseFloat(survivorElect)===pct?"#3e3828":"#302f29",
                        color:parseFloat(survivorElect)===pct?"#d4a843":"#c4b99a",
                        fontFamily:"'DM Sans',sans-serif"
                      }}>{pct===66.67?"66⅔":pct}%</button>
                    ))}
                  </div>
                  <div style={{fontSize:10,color:"#636054",marginTop:6}}>
                    {plan.name} default is {plan.survivorPct===66.67?"66⅔":plan.survivorPct}%. Higher options reduce your monthly pension.
                    {plan.name==="OMERS"?" OMERS does not offer increased survivor options.":""}
                  </div>
                </div>
              )}
              {plan&&plan.survivorOptions.length===1&&(
                <div style={{marginBottom:20,fontSize:12,color:"#858070"}}>
                  {plan.name} survivor benefit is fixed at {plan.survivorPct===66.67?"66⅔":plan.survivorPct}% — no election available.
                </div>
              )}

              <div style={{display:"flex",flexWrap:"wrap",gap:"20px 24px"}}>
                <Input label="Annual investment fees" value={investFees} onChange={setInvestFees} prefix="%" tooltip="ETF portfolios: 0.2-0.5%. Mutual funds: 1-2%. Robo-advisors: 0.4-0.7%." hint="Deducted from your return rate"/>
                <Input label="Non-registered tax drag" value={taxDrag} onChange={setTaxDrag} prefix="%" tooltip="Annual tax on investment income in a non-registered account. Dividends, interest, and realized gains are taxed yearly." hint="Applied to cash-out scenario only"/>
              </div>
            </div>
          )}
          ...
          {showAdvanced&&(
            <div style={{padding:24,background:"#38362e",borderRadius:12,border:"1px solid #403e3a"}}>
              ...
              <div style={{marginBottom:24}}>
                ...
                {plan&&plan.colaType==="conditional"&&(<div style={{fontSize:11,color:"#d4a843",marginTop:6,lineHeight:1.4}}>{plan.name}'s COLA is not guaranteed — provided at the plan's discretion based on funding.</div>)}
                {plan&&plan.colaType==="partial"&&(<div style={{fontSize:11,color:"#d4a843",marginTop:6,lineHeight:1.4}}>{plan.name}'s COLA is guaranteed for older service but conditional for newer service.</div>)}
                {plan&&plan.colaType==="guaranteed"&&(<div style={{fontSize:11,color:"#4ecdc4",marginTop:6,lineHeight:1.4}}>{plan.name} guarantees annual COLA adjustments.</div>)}
              </div>
              <div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          ...
          <button onClick={()=>{if(ready){setShowResults(true);setTimeout(()=>document.getElementById("results")?.scrollIntoView({behavior:"smooth"}),100)}}}
            ...
            style={{
              ...
              fontFamily:"'DM Sans',sans-serif",transition:"all 0.2s",
              boxShadow:ready?"0 4px 20px rgba(212,168,67,0.25)":"none",letterSpacing:"0.02em"
            }}>
        ...
        {showResults&&R&&(
          <div id="results">
            ...
            <div style={{background:"#2b2a25",border:"1px solid #403e3a",borderRadius:14,padding:28,marginBottom:28,textAlign:"center"}}>
              <div style={{fontSize:11,fontWeight:600,color:"#d4a843",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:10,fontFamily:"'DM Mono',monospace"}}>{isDetailed?"Detailed":"Quick"} Analysis</div>
              <div style={{fontSize:15,color:"#8a8270",lineHeight:1.7}}>
                ...
                {R.yToR>0?` Your pension begins in ${R.yToR} years.`:""}
                {isDetailed&&R.rrspSheltered>0?` ${fmt(R.rrspSheltered)} of your excess sheltered in RRSP.`:""}
                {isDetailed&&spouseA>0?` Survivor benefit modeled for spouse age ${spouseA}.`:""}
              </div>
            </div>
            ...
            <div style={{background:"#2b2a25",border:"1px solid #403e3a",borderRadius:14,padding:28,marginBottom:28}}>
              ...
              {[{l:"Keep pension",v:R.dR+(R.survivorValue||0),c:"#4ecdc4"},...(cvEligible?[{l:"Transfer & invest",v:R.iR,c:"#d4a843"},{l:"Cash out & invest",v:R.cR,c:"#e07a5f"}]:[])].map((s,i)=>(
              ...
              {R.bk&&(
                <div style={{marginTop:18,padding:"12px 16px",background:"#2c3c32",border:"1px solid #3e5f4f",borderRadius:10,fontSize:13,color:"#4ecdc4",lineHeight:1.5}}>
                  ...
                  {plan.colaType==="guaranteed"?` Since ${plan.name} guarantees COLA, this advantage is protected against inflation.`
                    :plan.colaType==="partial"?` Note: ${plan.name}'s COLA is partially guaranteed. Actual increases on newer service may vary.`
            ...
            {!cvEligible&&(
              <div style={{background:"#3c2c2c",border:"1px solid #6b4444",borderRadius:12,padding:"20px 24px",marginBottom:20}}>
                <div style={{fontSize:14,fontWeight:700,color:"#e07a5f",marginBottom:8}}>Transfer not available</div>
                <div style={{fontSize:13,color:"#c4b99a",lineHeight:1.6}}>
                  {plan.name} does not allow commuted value transfers at your age ({a}). You must be under age {plan.cvMaxAge} to transfer out.
                  {plan.hasMembershipExtension?" Note: CAAT has a 24-month membership extension — options are finalized 24 months after your last contribution.":""}
                  {" "}Your option is to keep your deferred pension with {plan.name} and begin collecting at age {plan.earliestRetAge} or later.
                </div>
              </div>
            )}

            {cvEligible&&plan?.hasInsuredBenefits&&(
              <div style={{background:"#3c3422",border:"1px solid #6b5c2e",borderRadius:12,padding:"20px 24px",marginBottom:20}}>
                <div style={{fontSize:14,fontWeight:700,color:"#d4a843",marginBottom:8}}>Insured benefits at risk</div>
                <div style={{fontSize:13,color:"#c4b99a",lineHeight:1.6}}>
                  {plan.name} members who keep their pension may be eligible for Government of Ontario post-retirement health, dental, and life insurance benefits. Transferring your commuted value means you permanently lose eligibility for these benefits, which can be worth $3,000–$8,000 per year in retirement. Confirm your eligibility with your employer.
                </div>
              </div>
            )}

            {cvEligible&&plan?.hasMembershipExtension&&(
              <div style={{background:"#3c3422",border:"1px solid #6b5c2e",borderRadius:12,padding:"20px 24px",marginBottom:20}}>
                <div style={{fontSize:14,fontWeight:700,color:"#d4a843",marginBottom:8}}>CAAT 24-month membership extension</div>
                <div style={{fontSize:13,color:"#c4b99a",lineHeight:1.6}}>
                  CAAT extends your membership for 24 months after you leave. Your termination options — including the commuted value — are not finalized until this period ends. The actual CV may differ from this estimate due to changing interest rates.
                </div>
              </div>
            )}

            {cvEligible&&(
              <div style={{background:"#2b2a25",border:"1px solid #403e3a",borderRadius:12,padding:"20px 24px",marginBottom:20}}>
                <div style={{fontSize:13,fontWeight:700,color:"#c4b99a",marginBottom:8}}>What you give up by transferring</div>
                <div style={{fontSize:12,color:"#8a8270",lineHeight:1.7}}>
                  Guaranteed lifetime income · {plan.colaType==="guaranteed"?"Guaranteed":plan.colaType==="partial"?"Partially guaranteed":"Conditional"} inflation protection · {plan.survivorPct===66.67?"66⅔":plan.survivorPct}% survivor pension for your spouse · Bridge benefit to age {plan.bridgeEndAge}{plan.hasInsuredBenefits?" · Eligibility for Government of Ontario post-retirement insured benefits":""} · Protection against outliving your savings · No investment decisions required
                </div>
              </div>
            )}

            {resultsClose&&cvEligible&&(
              <div style={{background:"#2c3c32",border:"1px solid #3e5f4f",borderRadius:12,padding:"20px 24px",marginBottom:20}}>
                <div style={{fontSize:13,fontWeight:700,color:"#4ecdc4",marginBottom:6}}>Results are close</div>
                <div style={{fontSize:12,color:"#8a8270",lineHeight:1.7}}>
                  The top options are within 15% of each other. Small changes in assumptions — investment returns, life expectancy, inflation — could change which option comes out ahead. Consider consulting a licensed financial planner before deciding.
                </div>
              </div>
            )}
                ...
                <div style={{display:"flex",gap:10,maxWidth:440,margin:"0 auto",flexWrap:"wrap"}}>
                  ...
                  <button onClick={()=>{
                    ...
                    fetch("https://formspree.io/f/xkoqnpnj",{
                      ...
                      body:JSON.stringify({email,plan:plan?.name||"none",age:age||"",cv:cv||"",mode})
                    }).then(r=>{if(r.ok)setEmailSent(true)}).catch(()=>setEmailSent(true));
                  }} style={{padding:"12px 24px",background:"#d4a843",color:"#24231e",border:"none",borderRadius:10,fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap",flex:"0 0 auto"}}>Notify Me</button>
                </div>
            ...
            {/* ═══ PLANNER CTA ═══ */}
            <div id="planner" style={{background:"linear-gradient(135deg,#38362e,#2b2a25)",border:"1px solid #524f44",borderRadius:14,padding:"36px 24px",marginBottom:28,textAlign:"center"}}>
              <div style={{fontSize:22,fontWeight:800,fontFamily:"'Playfair Display',serif",marginBottom:12,lineHeight:1.3}}>This decision is irreversible.</div>
              ...
              <a href="https://maketheswitch.ca" target="_blank" rel="noopener noreferrer" style={{
                ...
                boxShadow:"0 4px 20px rgba(212,168,67,0.3)",letterSpacing:"0.02em",maxWidth:"100%",boxSizing:"border-box"
              }}>Find a Fee-Only Planner →</a>
              <div style={{fontSize:12,color:"#858070",marginTop:16}}>We only recommend fee-only planners who do not earn commissions on your decision.</div>
            </div>
        ...
        <div style={{textAlign:"center",padding:"32px 0"}}>
          <a href="https://buymeacoffee.com/pensionpath" target="_blank" rel="noopener noreferrer" style={{
            display:"inline-flex",alignItems:"center",gap:6,padding:"10px 20px",
            background:"#FFDD00",color:"#000",border:"none",borderRadius:8,
            fontSize:14,fontWeight:700,fontFamily:"'DM Sans',sans-serif",
            textDecoration:"none",lineHeight:1
          }}>☕ Buy me a coffee</a>
          <div style={{fontSize:11,color:"#636054",fontFamily:"'DM Mono',monospace",marginTop:16}}>
